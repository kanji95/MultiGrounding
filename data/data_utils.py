...
class Data():
    def __init__(self):
        self.supported = ['flickr30k_entities',
                          'coco', 'visual_genome', 'referit']
            ...
            if split_s == '':
                ...
                if obj_id == '0':
                    ...
                    for bbox in bboxes:
                        ...
                        bboxes_norm.append(
                            [bb_x_n[0], bb_y_n[0], bb_x_n[1], bb_y_n[1]])
                ...
                if obj.tag == 'size':
                    ...
                    for item in obj.iter('bndbox'):
                        for obj_id in obj_ids:
                            tmp_dict[obj_id].append(
                                [int(bb.text) for bb in item])
    ...
    def _flickr30k(self, raw_path, save_path, store_lmdb):
        ...
        for split in ['train', 'test', 'val']:
            ...
            with open(split_path+split+'.txt', 'r') as f:
                ...
                lines = [l.strip('\n') for l in lines]
            keys = set(lines)
            dict_ = self._get_dict_flickr30k(
                keys, xml_files, annot_path, sen_path, imgs_path, store_lmdb)
            pickle.dump(dict_, open(annot_dict_path+split+'.pickle',
    ...
    def _get_split_coco(self, annot_path, split, version):
        ...
        instancesAnnFile = '{}/instances_{}{}.json'.format(
            annot_path, split, version)
        captionsAnnFile = '{}/captions_{}{}.json'.format(
            annot_path, split, version)
    ...
    def _get_dict_coco(self, imgs_path, coco_obj, image_objects, caption_objects, names, supercats, store_lmdb):
        ...
        for n, img_obj in enumerate(image_objects):
            img_id, height, width, filename = (
                img_obj['id'], img_obj['height'], img_obj['width'], img_obj['file_name'])

            if store_lmdb:
                image = cv2.imread(os.path.join(
                    imgs_path, filename), cv2.IMREAD_COLOR)
    ...
    def _coco(self, raw_path, save_path, store_lmdb, version):
        ...
        for split in ['train', 'val']:  # we only process train and val splits
            print('Processing '+split+' split...')
            coco_obj, image_objects, caption_objects, names, supercats = self._get_split_coco(
                annot_path, split, version)
            dict_ = self._get_dict_coco(
                imgs_path, coco_obj, image_objects, caption_objects, names, supercats, store_lmdb)
            pickle.dump(dict_, open(annot_dict_path+split+'.pickle',
                                    'wb'), protocol=pickle.HIGHEST_PROTOCOL)
            if split == 'train':
                ...
                pickle.dump(train_val_dict_, open(
                    annot_dict_path+'train_val.pickle', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
            print('\n')
        print('Processing done.')
    ...
    def _get_dict_refer(self, refer_obj, img_ids_all, img_ids_split):
        ...
        for n, img_id in enumerate(img_ids_split):
            ...
            for ref in refs:
                ...
                x1, x2, y1, y2 = [bbox[0], bbox[0] +
                                  bbox[2], bbox[1], bbox[1]+bbox[3]]
    ...
    def _get_dict_vg(self, imgs_path, img_ids_split, imgs_data, regions_data, store_lmdb):
        ...
        for n, img_id in enumerate(img_ids_split):
            ...
            for region in regions:
                ...
                bbox_norm = [bbox[0]/width, bbox[1] /
                             height, bbox[2]/width, bbox[3]/height]
    ...
    def _vg(self, raw_path, save_path, store_lmdb):
        ...
        imgs_data, regions_data, img_ids_split = self._get_meta_vg(
            anns_path, splits_path)

        for split in ['train', 'test']:
            print('Processing '+split+' split...')
            dict_ = self._get_dict_vg(
                imgs_path, img_ids_split[split], imgs_data, regions_data, store_lmdb)
            pickle.dump(dict_, open(annot_dict_path+split+'.pickle',
                                    'wb'), protocol=pickle.HIGHEST_PROTOCOL)
            print('\n')
        print('Processing done.')
