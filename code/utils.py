...
def heat2bbox(heat_map, original_image_shape):
    ...
    for iii in range(len(bboxes)):
        for iiii in range(iii):
            ...
            b1 = bboxes[iii]
            b2 = bboxes[iiii]
            isec = max(min(b1[2], b2[2]) - max(b1[0], b2[0]), 0) * \
                max(min(b1[3], b2[3]) - max(b1[1], b2[1]), 0)
        ...
        for i in range(len(bboxes)):
            ...
            if order == 'xxyy':
                x_min, x_max, y_min, y_max = int(
                    bbox_norm[0]*W), int(bbox_norm[1]*W), int(bbox_norm[2]*H), int(bbox_norm[3]*H)
            elif order == 'xyxy':
                x_min, x_max, y_min, y_max = int(
                    bbox_norm[0]*W), int(bbox_norm[2]*W), int(bbox_norm[1]*H), int(bbox_norm[3]*H)
            x_length, y_length = x_max-x_min, y_max-y_min
            box = plt.Rectangle((x_min, y_min), x_length,
                                y_length, edgecolor='w', linewidth=3, fill=False)
...
def crop_resize_im(image, bbox, size, order='xxyy'):
    H, W, _ = image.shape
    if order == 'xxyy':
        roi = image[int(bbox[2]*H):int(bbox[3]*H),
                    int(bbox[0]*W):int(bbox[1]*W), :]
    elif order == 'xyxy':
        roi = image[int(bbox[1]*H):int(bbox[3]*H),
...
class pre_trained_load():
    ...
    def __init__(self, model_name, image_shape=(None, 224, 224, 3),
                 input_tensor=None, session=None, is_training=False,
                 global_pool=False, num_classes=None):
        supported_list = ['vgg_16', 'InceptionV3', 'InceptionV4',
                          'pnasnet_large', 'resnet_v2_152', 'resnet_v2_101', 'resnet_v2_50']
    ...
    def _build_graph(self, input_tensor):
        with tf.name_scope('inputs'):
            if input_tensor is None:
                input_tensor = tf.placeholder(
                    tf.float32, shape=self.image_shape, name='input_img')
            else:
        ...
        if self.model_name == 'vgg_16':
            ...
            from models.visual_models.nets.vgg import vgg_16, vgg_arg_scope
            with slim.arg_scope(vgg_arg_scope()):
                self.output, self.outputs = vgg_16(
                    self.input_tensor, num_classes=self.num_classes, is_training=self.is_training, global_pool=self.global_pool)
                self.outputs = OrderedDict(
                    [(k.split('/')[-1], v) for k, v in self.outputs.items()])

        if self.model_name == 'resnet_v2_152':
            self.ckpt_path = slim_models_path+"resnet_v2_152.ckpt"
            from models.visual_models.nets.resnet_v2 import resnet_v2_152, resnet_arg_scope
            ...
            from models.visual_models.nets.resnet_v2 import resnet_v2_101, resnet_arg_scope
            with slim.arg_scope(resnet_arg_scope()):
            ...
            from models.visual_models.nets.resnet_v2 import resnet_v2_50, resnet_arg_scope
            with slim.arg_scope(resnet_arg_scope()):
                self.output, self.outputs = resnet_v2_50(
                    self.input_tensor, num_classes=self.num_classes, is_training=self.is_training, global_pool=self.global_pool)

        elif self.model_name == 'InceptionV3':
            self.ckpt_path = slim_models_path+"InceptionV3.ckpt"
            from models.visual_models.nets.inception import inception_v3, inception_v3_arg_scope
            with slim.arg_scope(inception_v3_arg_scope()):
                self.output, self.outputs = inception_v3(
                    self.input_tensor, num_classes=self.num_classes, is_training=self.is_training)

        elif self.model_name == 'InceptionV4':
            self.ckpt_path = slim_models_path+"InceptionV4.ckpt"
            from models.visual_models.nets.inception import inception_v4, inception_v4_arg_scope
            with slim.arg_scope(inception_v4_arg_scope()):
                self.output, self.outputs = inception_v4(
                    self.input_tensor, num_classes=self.num_classes, is_training=self.is_training)

        elif self.model_name == 'pnasnet_large':
            self.ckpt_path = slim_models_path+"pnasnet_large.ckpt"
            from models.visual_models.nets.nasnet.pnasnet import build_pnasnet_large, pnasnet_large_arg_scope
...
def isCorrectHit(bbox_annot, heatmap, orig_img_shape):
    ...
    max_loc = np.unravel_index(
        np.argmax(heatmap_resized, axis=None), heatmap_resized.shape)
...
def calc_correctness(annot, heatmap, orig_img_shape):
    ...
    bbox, bbox_norm, bbox_score = filter_bbox(
...
def checkpoint_modify_prefix(ckpt_path, old_prefix, new_prefix, new_ckpt_path, saver_ver=tf.train.SaverDef.V2):
    vars = tf.contrib.framework.list_variables(ckpt_path)
    new_vars = []
    for old_name, shape in vars:
        new_name = old_name.replace(old_prefix, new_prefix)
        v = tf.contrib.framework.load_variable(ckpt_path, old_name)
        new_vars.append(tf.Variable(v, shape=shape, name=new_name))

    model_saver = tf.train.Saver(var_list=new_vars, write_version=saver_ver)
    sess = tf.InteractiveSession()
    sess.run(tf.global_variables_initializer())
    model_saver.save(sess, new_ckpt_path)


def checkpoint_add_prefix(ckpt_path, prefix, new_ckpt_path, saver_ver=tf.train.SaverDef.V2):
    vars = tf.contrib.framework.list_variables(ckpt_path)
    new_vars = []
    for old_name, shape in vars:
