...
class gnet_inference():
    def __init__(self,
        ...
        self.sess, _ = load_model(self.ckpt_path, self.config)
...
class LoadGroundNet():
    ...
    def _load_gnet(self):
        ...
        #print(f'graph: {[str(op.name) for op in self.sess.graph.get_operations()]}', flush=True)
        #print(f'graph: {self.sess.graph_def}', flush=True)
        #self.sess, _ = load_model(self.ckpt_path, config)
        self.end_points['image_input'] = self.sess.graph.get_tensor_by_name(
            "input_img:0")
        self.end_points['text_input'] = self.sess.graph.get_tensor_by_name(
            "text_input:0")
        self.end_points['mode'] = self.sess.graph.get_tensor_by_name("mode:0")
        self.end_points['score_word'] = self.sess.graph.get_tensor_by_name(
            "attention/score_word:0")
        self.end_points['score_sentence'] = self.sess.graph.get_tensor_by_name(
            "attention/score_sentence:0")
        self.end_points['heatmap_word'] = self.sess.graph.get_tensor_by_name(
            "attention/heatmap_word:0")
        self.end_points['heatmap_sentence'] = self.sess.graph.get_tensor_by_name(
            "attention/heatmap_sentence:0")
        ...
        self.end_points['level_index_word'] = self.sess.graph.get_tensor_by_name(
            'attention/level_index_word:0')
        self.end_points['level_score_word'] = self.sess.graph.get_tensor_by_name(
            'attention/level_score_word:0')
        self.end_points['level_index_sentence'] = self.sess.graph.get_tensor_by_name(
            'attention/level_index_sentence:0')
        print('Loading done.')
